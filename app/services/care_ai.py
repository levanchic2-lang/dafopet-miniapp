...
async def draft_client_care_summary(payload: dict[str, Any], doctor_instruction: str = "") -> dict[str, Any]:
    ...
    text = _strip_md(resp.choices[0].message.content or "")
...
async def draft_care_plan(payload: dict[str, Any], doctor_instruction: str = "") -> dict[str, Any]:
    ...
    for t in tasks[:4]:
        ...
        cleaned.append({
            ...
            "task_type": task_type,
            "title": str(t.get("title") or "复诊回访").strip()[:80],
            "question_text": str(t.get("question_text") or "").strip(),
            "risk_trigger": str(t.get("risk_trigger") or "").strip(),
            "priority": priority,
        })
    ...
    return {
        "ok": True,
        "title": str(data.get("title") or "复诊计划").strip()[:160],
